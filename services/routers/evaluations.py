...
async def get_calculation_result(calculation_id: int, db: DatabaseConnection = Depends(get_database_connection)) -> Any:
    dao = CalculationResultDAO(db)
    record = await dao.get_by_id(calculation_id)
    if not record:
...
async def create_calculation_result(payload: CalculationResultCreate, db: DatabaseConnection = Depends(get_database_connection)) -> Any:
    dao = CalculationResultDAO(db)
    calculation_id = await dao.create_from_model(payload)
...
async def update_calculation_result(
    ...
    dao = CalculationResultDAO(db)
    if not await dao.update(calculation_id, payload):
        raise HTTPException(status_code=404, detail="Calculation result not found")
    return {"calculation_id": calculation_id}


@router.delete("/calculation-results/{calculation_id}", status_code=204)
async def delete_calculation_result(calculation_id: int, db: DatabaseConnection = Depends(get_database_connection)) -> Response:
    dao = CalculationResultDAO(db)
    if not await dao.delete_by_id(calculation_id):
        raise HTTPException(status_code=404, detail="Calculation result not found")
...
async def get_evaluation_result(evaluation_result_id: int, db: DatabaseConnection = Depends(get_database_connection)) -> Any:
    dao = EvaluationResultDAO(db)
    record = await dao.get_by_id(evaluation_result_id)
    if not record:
...
async def create_evaluation_result(payload: EvaluationResultCreate, db: DatabaseConnection = Depends(get_database_connection)) -> Any:
    dao = EvaluationResultDAO(db)
    evaluation_result_id = await dao.create_from_model(payload)
...
async def update_evaluation_result(
    ...
    dao = EvaluationResultDAO(db)
    if not await dao.update(evaluation_result_id, payload):
        raise HTTPException(status_code=404, detail="Evaluation result not found")
    return {"evaluation_result_id": evaluation_result_id}


@router.delete("/evaluation-results/{evaluation_result_id}", status_code=204)
async def delete_evaluation_result(evaluation_result_id: int, db: DatabaseConnection = Depends(get_database_connection)) -> Response:
    dao = EvaluationResultDAO(db)
    if not await dao.delete_by_id(evaluation_result_id):
        raise HTTPException(status_code=404, detail="Evaluation result not found")
    return Response(status_code=204)
...
async def get_evaluation_test_case(test_case_id: int, db: DatabaseConnection = Depends(get_database_connection)) -> Any:
    dao = EvaluationTestCaseDAO(db)
    record = await dao.get_by_id(test_case_id)
    if not record:
...
async def create_evaluation_test_case(payload: EvaluationTestCaseCreate, db: DatabaseConnection = Depends(get_database_connection)) -> Any:
    dao = EvaluationTestCaseDAO(db)
    test_case_id = await dao.create_from_model(payload)
...
async def update_evaluation_test_case(
    ...
    dao = EvaluationTestCaseDAO(db)
    if not await dao.update(test_case_id, payload):
        raise HTTPException(status_code=404, detail="Evaluation test case not found")
    return {"test_case_id": test_case_id}


@router.delete("/evaluation-test-cases/{test_case_id}", status_code=204)
async def delete_evaluation_test_case(test_case_id: int, db: DatabaseConnection = Depends(get_database_connection)) -> Response:
    dao = EvaluationTestCaseDAO(db)
    if not await dao.delete_by_id(test_case_id):
        raise HTTPException(status_code=404, detail="Evaluation test case not found")
    return Response(status_code=204)
